...
/*
 * Test UDP Port and given STM32 IP Address
 */
...
volatile uint8_t dbg_arp_sender_ip0 = 0U;
volatile uint8_t dbg_arp_sender_ip1 = 0U;
volatile uint8_t dbg_arp_sender_ip2 = 0U;
volatile uint8_t dbg_arp_sender_ip3 = 0U;

volatile uint8_t dbg_arp_target_ip0 = 0U;
volatile uint8_t dbg_arp_target_ip1 = 0U;
volatile uint8_t dbg_arp_target_ip2 = 0U;
volatile uint8_t dbg_arp_target_ip3 = 0U;
			...
			if(ETHNET_ParseEthernetFrame(frame, frame_length, &eth_frame) == 1U){
				...
				if(eth_frame.frame_format == ETHNET_FRAME_ETHERNET_II){
					if(eth_frame.ethertype == ETHNET_ETHERTYPE_IPV4){
						...
						//Parsing ARP Frame
						if(ETHNET_ParseARPPacket(eth_frame.payload, eth_frame.payload_len, &arpPacket) == 1U){
							dbg_ethnet_arp_count++;

							dbg_arp_sender_ip0 = arpPacket.sender_ip[0];
							dbg_arp_sender_ip1 = arpPacket.sender_ip[1];
							dbg_arp_sender_ip2 = arpPacket.sender_ip[2];
							dbg_arp_sender_ip3 = arpPacket.sender_ip[3];

							dbg_arp_target_ip0 = arpPacket.target_ip[0];
							dbg_arp_target_ip1 = arpPacket.target_ip[1];
							dbg_arp_target_ip2 = arpPacket.target_ip[2];
							dbg_arp_target_ip3 = arpPacket.target_ip[3];


							if(arpPacket.oper == ETHNET_ARP_OPER_REQUEST){
								dbg_arp_request_count++;
							} else if(arpPacket.oper == ETHNET_ARP_OPER_REPLY){
								dbg_arp_reply_count++;
							}

							if(ETHNET_IsARPRequestForIP(&arpPacket, myIP) == 1U){
